...
public:
  ...
  Stack(Stack&& other) noexcept
      : head(other.head), currentSize(other.currentSize) {
    other.head = nullptr;
    other.currentSize = 0;
  }
  ...
  */
  ...
  Stack& operator=(const Stack& other) {
      ...
      // Create a temporary deep copy. If 'new' fails here, the original object
  remains untouched. Stack temp(other);

      // Swap pointers and sizes
      swap(head, temp.head);
      swap(currentSize, temp.currentSize);

      // temp's destructor will automatically clean up the old memory of this
  object return *this;
  }
  */
...
int main() {
  try {
    ...
    Stack s3;
    s3.push(999);
    s3 = s1;  // The old memory of s3 (999) is safely destroyed by temp's
              // destructor
    ...
    cout << "s2 Size after move: " << s2.size()
         << " (Empty: " << (s2.empty() ? "Yes" : "No") << ")" << endl;
    ...
    cout << "s3 Size after move: " << s3.size()
         << " (Empty: " << (s3.empty() ? "Yes" : "No") << ")" << endl;

    // Custom Friend Swap Test (Outside Call)
    s1.push(555);
    cout << "\n--- Direct Friend Swap Test (s1 <-> s5) ---" << endl;
    cout << "Before Swap -> s1 Top: " << s1.top() << ", s5 Top: " << s5.top()
         << endl;
    swap(s1, s5);  // Direct non-member call
    cout << "After Swap  -> s1 Top: " << s1.top() << ", s5 Top: " << s5.top()
         << endl;
