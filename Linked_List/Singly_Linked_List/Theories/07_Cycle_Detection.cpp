...
bool createCycle(Node* head, int startIdx, int endIdx) {
  ...
  while (temp != nullptr) {
    if (currIdx == startIdx) startNode = temp;
    if (currIdx == endIdx) endNode = temp;
    if (startNode != nullptr && endNode != nullptr) break;
    temp = temp->next;
    currIdx++;
  }
  ...
  cout << "Cycle is created successfully between index " << startIdx
       << " to index " << endIdx << endl;
...
int main() {
  ...
  if (hasCycle(head)) cout << "A cycle detected..." << endl;
