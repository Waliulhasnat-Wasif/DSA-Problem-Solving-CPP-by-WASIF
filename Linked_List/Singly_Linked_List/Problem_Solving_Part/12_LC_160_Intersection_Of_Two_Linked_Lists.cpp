...
void createIntersectedLists(const vector<int>& valsA, const vector<int>& valsB,
                            int skipA, int skipB, ListNode*& headA,
                            ListNode*& headB, ListNode*& intersectNode) {
  headA = nullptr;
  headB = nullptr;
  ...
  if (targetA != nullptr && targetB != nullptr && skipA < valsA.size() &&
      skipB < valsB.size()) {
...
void runTestCase(const vector<int>& valsA, const vector<int>& valsB, int skipA,
                 int skipB, const string& testName) {
  Solution sol;
  ...
  createIntersectedLists(valsA, valsB, skipA, skipB, headA, headB,
                         expectedNode);
...
int main() {
  ...
  runTestCase({4, 1, 8, 4, 5}, {5, 6, 1, 8, 4, 5}, 2, 3,
