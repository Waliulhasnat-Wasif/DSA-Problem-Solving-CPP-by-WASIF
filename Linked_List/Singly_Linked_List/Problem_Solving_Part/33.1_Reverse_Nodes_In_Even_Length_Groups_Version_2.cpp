...
void runTestCase(const vector<int>& input, bool reverseEven,
                 const string& testName) {
  Solution sol;

  cout << "Test Case: " << testName << "\n";
  cout << "Mode       : "
       << (reverseEven ? "Reverse EVEN lengths" : "Reverse ODD lengths")
       << "\n";
...
int main() {
  ...
  runTestCase({5, 2, 6, 3, 9, 1, 7, 3, 8, 4}, true,
              "Even Mode - Complex group lengths");
  runTestCase({1, 1, 0, 6}, true, "Even Mode - Last group is odd length (1)");

  runTestCase({5, 2, 6, 3, 9, 1, 7, 3, 8, 4}, false,
              "Odd Mode - Complex group lengths");
